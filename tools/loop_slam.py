...
(DRIVE_FWD_SIGN). Нужен torch+transformers (HF_HUB_OFFLINE=1), близко к роутеру.
Запуск:  python tools/loop_slam.py [IP] [--fwd 4] [--pwm 150]
"""
import io
import json
import math
...
def main():
    ...
    try:
        ...
        for s in range(FWD_STEPS):
            if c["fwd"] and 0 < c["fwd"] < STOP_MM:
                ...
                break
            burst(DRIVE_FWD_SIGN)
            c = capture(backend)
            if c is None or not add_kf(c):
                break
            fwd_done += 1
        # назад столько же (возврат в старт → петля)
    ...
    if loops == 0:
        ...
        print("\nОПТИМИЗАЦИЯ: ошибка %.3f→%.3f | возврат в старт %.2f→%.2f м (петля %s)"
              % (e0, e1, drift_before, drift_after, "ЗАМКНУТА" if drift_after < drift_before * 0.6 else "слабо"))
