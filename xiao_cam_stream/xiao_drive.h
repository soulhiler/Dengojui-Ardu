 ...
 *
 * Слитая версия (port claude/dev -> main):
 *  - одометрия (spd_l/spd_r), антидребезг энкодеров, рефлекс безопасности
 *    (бампер + УЗ-стоп, safety-битмаска перекрывает команды) — из claude/dev;
 *  - аудио-хуки (xiaoAudioIsActive/xiaoAudioStop) — из main (motor-audio).
 *  Все сенсорные ветки инертны, пока пины в drive_config.h = 0.
 */
...
#endif
/* Дефолты на случай drive_config.h без этих ключей (например main-конфиг). */
...
static void driveApplyMotors() {
  // Аудио через моторы (motor-audio) важнее привода: пока пищим — не крутим.
...
static inline void xiaoDriveSetLr(int16_t l, int16_t r) {
  // Любая команда привода прерывает звук через моторы (motor-audio).
#if defined(XIAO_AUDIO_ENABLE) && XIAO_AUDIO_ENABLE
