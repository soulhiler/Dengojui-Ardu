...
/** Регулятор скорости по фронтальному ToF (B10 «регулятор, не реле»): плавно
 *  сбавляет ход ВПЕРЁД при приближении к препятствию (дальше SLOW — полный газ,
 *  между — линейно, ближе STOP — стоп вперёд). Реверс/поворот не трогаются;
 *  дистанция 0 (нет цели) не ограничивает. Берёт tof_mm (центр-полоса).
 *  0 — выключить регулятор. */
#define XIAO_DRIVE_TOF_GOV 1
#define XIAO_DRIVE_GOV_STOP_MM 150  /* ближе — стоп вперёд */
#define XIAO_DRIVE_GOV_SLOW_MM 600  /* дальше — без ограничения */
