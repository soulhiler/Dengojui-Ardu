...
| CS | D10 |
| DC | D12 |
| **RES** | **D6** (импульс сброса в прошивке; без этого экран часто «мёртвый») |

**Не подключайте OLED на D7/D8** — там мотор B.

По умолчанию в прошивке драйвер **SSD1306**. Если строка `drv SSD1306` не появляется, а раньше были «полосы» — в `arduino_motor_test.ino` поставьте `OLED_USE_SH1106 = 1` и перешейте.

На экране текст: **L / R**, **VM** (делитель на **A0**), **STBY**, режим драйвера.

Делитель VM → A0: верх **20 kΩ**, низ **10 kΩ** (`VDIV_SCALE`).
